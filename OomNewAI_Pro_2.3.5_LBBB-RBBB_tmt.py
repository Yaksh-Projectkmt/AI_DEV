...
#MQTT Topics
topic_x =  "$share/python/oom/ecg/tmt/rawData"
topic_y = "oom/ecg/tmt/processedData"
##topic_x = "oom/ecg/rawData"
##topic_y = "oom/1"

broker = 'oomcardiodev.projectkmt.com'
port = 8883
client_id = f'python-mqtt-{random.randint(1000000000, 2000000000)}'
username = 'kmt'
password = 'Kmt123'
...
interpreterss = tf.lite.Interpreter(model_path='PVC_Trans_mob_15.tflite')
...
##    load_model = tf.saved_model.load('PVC_PHYSICS_11')
##    afib_load_model = tf.saved_model.load('AFIB_AFL_18')
    afib_load_model = load_tflite_model("afib_flutter_28_6.tflite")
    ...
    pac_load_model = load_tflite_model("PAC_TRANS_GRU_Res_8.tflite")
    block_load_model = load_tflite_model("block_3_9.tflite")
    let_inf_moedel = load_tflite_model("ST_5.tflite")
...
def afib_percentage(p_t, fs=200):
    ...
    """Calculates the percentage of atrial fibrillation (AFib) in a given patient's
    electrocardiogram (ECG) data based on the duration of irregular heartbeats.

    Args:
        p_t (list of lists): A list of lists, where each inner list represents the time intervals between consecutive
    heartbeats for a patient. The time intervals are in milliseconds

    Returns:
        _type_: The average percentage of AFib episodes based on the input parameter `p_t`, which is a list
    of lists containing the time intervals between consecutive R-peaks in an ECG signal.
    """
##    percentage = []
##    threshold = int(fs*0.35)
##    if fs != 200:
##        a = 1
##    else:
##        a = 2
...
##    """
##    Classify the ECG signal based on certain criteria.
##
##    Args:
##        p_t (list): It is a list of lists where each inner list represents a segment of ECG signal's P & T waves.
##
##    Returns:
##        str: a label indicating whether the signal is "Afib", "Abnormal", or "Normal".
##    """
##    total_len = len(p_t)
##    # Get the length of each inner list
##    lengths = []
...
##    threes_to_nines = sum([lengths.count(i) for i in range(3, 10)])
##    
##    # if round(((twos+ones)/total_len)*100)>=70 and round((ones/total_len)*100)>=50:
##    #     label = "Abnormal"
##    if (round((zeros/total_len)*100)>=17 and 
##        round((ones/total_len)*100)>=40 and 
##        round(((zeros+ones)/total_len)*100)>=40) or (round((threes_to_nines/total_len)*100) >= 6 and 
##                                                     afib_percentage(p_t,fs=fs)>=56):
##        label = "Afib"
##    elif round((twos/total_len)*100) >= 50:
##        label = "Abnormal"
##    elif round((zeros/total_len)*100) >= 70 or round((ones/total_len)*100) >= 70:
##        label = "Abnormal"
##    else:
##        label = "Abnormal"
##    return label, zeros, ones, twos, threes_to_nines
...
##        
##        # print(f"merged_r_peaks == {count}, p_peaks == {len(p_index)}, r_indices == {len(r_index)}")         
##        if ((len(r_index)-1 - (len(p_index)-count))/(len(r_index)-1))>=0.8:
##            jr_label = "Junctional_Rhythm" if hr_ >= 40 else "Junctional_Bradycardia"
##        else:
##            jr_label = "Abnormal"
##    return jr_label
...
# 2nd and 3rd degree block detection
##class BlockDetected:
##    def __init__(self, baseline_signal, r_index, hr_counts, p_t, pr_interval):
...
##        third_degree = []
##        possible_3rd = False
##        if self.hr_counts <= 60 and len(self.p_t) != 0: # 60
##            constant_2 = all(map(lambda innerlist: len(innerlist) == 2, self.p_t))
##            cons_2_1 = all(len(inner_list) in {1, 2} for inner_list in self.p_t)
##            ampli_val = list(map(lambda inner_list: sum(self.baseline_signal[i] > 0.05 for i in inner_list) / len(inner_list),self.p_t))
##            count_above_threshold = sum(1 for value in ampli_val if value > 0.7)
...
##                for inner_list in self.p_t:
##                    if len(inner_list) in [3, 4] :
##                        ampli_val = [self.baseline_signal[i] for i in inner_list] # ----------------New added
##                        if ampli_val  and (sum(value > 0.05 for value in ampli_val) / len(ampli_val)) > 0.7: 
##                            differences = np.diff(inner_list).tolist()
##                            diff_list = [x for x in differences if x >= 70]
##                            if len(diff_list) != 0:
...
##                            else:
##                                third_degree.append(0)    
##                    elif len(inner_list) in [3,4] and possible_3rd==True and constant_2 == False:
##                        differences = np.diff(inner_list).tolist()
##                        if all(diff > 70 for diff in differences):
##                            third_degree.append(1)
##                        else:
...
##                        third_degree.append(0)
##        if len(third_degree) != 0:
##            if third_degree.count(1) /len(third_degree) >= 0.5 : # or (possible_3rd == True and constant_2 == False) 0.4   
##                label = "3rd Degree block"
##        return label
##        
##    def second_degree_block_new(self):
...
##                                constant_3_peak.append(0)
##        if len(constant_3_peak) != 0 and constant_3_peak.count(1) != 0:
##            if constant_3_peak.count(1) /len(constant_3_peak) >= 0.5:
##                label = "Mobitz II"
##        elif possible_mob_1 and mob_count > 4:
##            label = "Mobitz I"
##        elif possible_mob_2 and mob_count > 4:
##            label = "Mobitz II"
##
##
##        return label
...
##    if  model_label == '1st_DEG' and abs_result == 'I DEGREE':
##        block_final_label = 'I DEGREE'
##    elif model_label == '2nd_DEG' and (abs_result == 'Mobitz I' or abs_result == 'Mobitz II'):
##        block_final_label = abs_result
##    elif model_label == '3rd_DEG' and abs_result == '3rd Degree block':
##        block_final_label = 'III Degree'
##    elif model_label == '2nd_DEG' and abs_result == '3rd Degree block':
##        block_final_label = 'Mobitz I'
##    elif model_label == '3rd_DEG' and (abs_result == 'Mobitz I' or abs_result == 'Mobitz II'):
##        block_final_label = 'III Degree'
...
# class MIDetection_MI:
#     def __init__(self, pd_data:pd.DataFrame, fs:int):
#         self.all_leads_data = pd_data
#         self.fs = fs
#         self.leads_pqrst_data = {}
#         self.mi_preprocessing()
        
#     def mi_preprocessing(self):
#         for lead in self.all_leads_data.columns:
#             lead_data = {}
#             ecg_signal = self.all_leads_data[lead].values
#             if self.fs != 200:
#                 ecg_signal = MinMaxScaler(feature_range=(0,2)).fit_transform(ecg_signal.reshape(-1,1)).squeeze()
#             baseline_signal, lowpass_signal = FilterSignal_MI(ecg_signal, fs=self.fs, MI=False).get_data()
#             pqrst_data = PQRSTDetection_MI(baseline_signal, fs=self.fs, MI=False).get_pqrst()
#             lead_data['p_index'], lead_data['q_index'] = pqrst_data['Ppeaks'], pqrst_data['Qpeaks']
#             lead_data['r_index'], lead_data['s_index'] = pqrst_data['Rpeaks'], pqrst_data['Speaks']
#             lead_data['t_index'] = pqrst_data['Tpeaks']
#             lead_data['t_onset'], lead_data['t_inverse'] = pqrst_data['Tonset'], pqrst_data['Tinverse']
#             lead_data['baseline_signal'] = baseline_signal
#             lead_data['lowpass_signal'] = lowpass_signal
#             st_data = STSegmentDetection(baseline_signal, lead_data['r_index'], lead_data['s_index'],
#                                          lead_data['q_index'], lead_data['t_index'], lead_data['t_onset'],
#                                          self.fs).get_data()
#             lead_data['ST_Label'], lead_data['ST_Area'] = st_data['label'], st_data['st_area']

#             self.leads_pqrst_data[lead] = lead_data
    
#     def check_inferior_STEMI(self, thresold=0.8):
#         lead_II = lead_III = lead_aVF = lead_aVL = 0
#         for lead in self.leads_pqrst_data.keys():
#             sub_data = self.leads_pqrst_data[lead]
#             sss_signal = sub_data['baseline_signal']
#             q_index, r_index = sub_data['q_index'], sub_data['r_index'] 
#             t_inverse_index = sub_data['t_inverse']
#             if 'II' == lead.upper():
#                 count = 0
#                 for q in q_index[:-1]:
#                     if ("ST Segment Elevation" in sub_data['ST_Label']):
#                         count += 1
#                 if count:
#                     if (len(r_index)-1)/count >= thresold:
#                         lead_II = 1
#             elif 'III' == lead.upper():
#                 count = 0
#                 for q in q_index[:-1]:
#                     if ("ST Segment Elevation" in sub_data['ST_Label']) and sss_signal[q] < 0:
#                         count += 1
#                 if count:
#                     if (len(r_index)-1)/count >= thresold:
#                         lead_III = 1
...
#             elif 'AVL' == lead.upper():
#                 count = 0
#                 for q, t1_ in zip(q_index[:-1], t_inverse_index):
#                     if ("ST Segment Depression" in sub_data['ST_Label']) or sss_signal[t1_] < 0:
#                         count += 1
#                 if count:
#                     if (len(r_index)-1)/count >= thresold:
#                         lead_aVL = 1
#         label = "Inferior STEMI" if (lead_II + lead_III + lead_aVF + lead_aVL) > 2 else 'Abnormal'
#         return label
       
#     def check_lateral_STEMI(self, thresold=0.8):
#         '''
...
#         '''
#         lead_I = lead_aVL = lead_V5 =  lead_III = lead_aVF = 0
#         for lead in self.leads_pqrst_data.keys():
#             sub_data = self.leads_pqrst_data[lead]
#             sss_signal = sub_data['baseline_signal']
#             q_index, r_index = sub_data['q_index'], sub_data['r_index'] 
...
#                 count = 0
#                 for q in q_index[:-1]:
#                     if ("ST Segment Depression" in sub_data['ST_Label']) and sss_signal[q] < 0:
#                         count += 1
#                 if count:
#                     if (len(r_index)-1)/count >= thresold:
#                         lead_III = 1
...
#                     if (len(r_index)-1)/count >= thresold:
#                         lead_V5 = 1
#         label = "Lateral STEMI" if (lead_I + lead_III + lead_aVF + lead_aVL + lead_V5) >= 3 else 'Abnormal'
#         return label
...
#     def check_LBBB_or_RBBB(self, ecg,rpeaks):
#         global folder_path
#         lis=[]
#         # ecg = pd.DataFrame(ecg)
#         count = 1
...
#             aa = pd.DataFrame(ecg[window_start:window_end]) # ecg.iloc
#             plt.plot(aa)
#             plt.axis("off")
#             plt.savefig(f"{folder_path}/p_{int(lis[0])}.jpg")            
#             aq = cv2.imread(f"{folder_path}/p_{int(lis[0])}.jpg")
#             aq = cv2.resize(aq, (360, 720))
#             cv2.imwrite(f"{folder_path}/p_{int(lis[0])}.jpg", aq)
#             lis.clear()
#             plt.close()
...
#         LBBB=[]
#         RBBB=[]
#         # files = self.sorting_key(glob.glob(imageresource+"*.jpg"))
#         for pvcfilename in glob.glob(folder_path+"/*.jpg"):
#             predictions,ids = self.prediction_model(pvcfilename)
#             # print(predictions,ids)
...
#     def anterior_posterior_detect(self, baseline_signal, r_index, Speaks, q_index, Tpeaks, frequency):
#         mi_counter = 0  # Counter for MI detections
#         label = "Abnormal"

#         if len(r_index) != 0:
#             for r_peak, t_peak in zip(r_index, Tpeaks):
#                 mid_value = (baseline_signal[r_peak] + baseline_signal[Speaks[Tpeaks.index(t_peak)]]) / 2  # Calculate mid value
                
#                 if baseline_signal[t_peak] > mid_value: # Compare T peak to mid value
#                     mi_counter += 1
    
#             mi_percentage = (mi_counter / len(r_index)) * 100
            
#             anterior = []
#             posterior = []
#             if mi_percentage >= 85:
#                 for i in range(len(r_index)-1):
#                     area = trapz(baseline_signal[r_index[i]:Tpeaks[i]+1])
#                     if round(area, 2) < 12:
#                         anterior.append(round(area,2))
#                     elif  12 <= round(area, 2) <= 30:
#                         posterior.append(round(area, 2))

#             anterior_per = (len(anterior)/len(r_index))*100
#             posterior_per = (len(posterior)/len(r_index))*100
#             if posterior_per >= 90:
#                 label = "Posterior"
#             elif anterior_per >= 90:
#                 label = "Anterior"
#             else:
#                 label = "Abnormal"
#         return label

#     def detect_inv_t(self, baseline_signal, r_index, Speaks, Qpeaks, Tpeaks):
#         inverted_t_indices = []
#         label = 'Abnormal'
#         for i in range(len(r_index)-1):
#             area = baseline_signal[Speaks[i]+10: Qpeaks[i+1]]
#             max_negative = np.min(area)
#             max_negative_index = np.where(area == max_negative)[0][0]
#             if max_negative < -0.1:
#                 if max_negative_index + Speaks[i] + 10 < Tpeaks[i] or Tpeaks[i] == 0:
#                     inverted_t_indices.append(max_negative_index + Speaks[i] + 10)
        
#         if len(inverted_t_indices) != 0:
#             if len(inverted_t_indices)/ len(r_index) >= 0.7:
#                 label = "Inverted T"
#         return label  #, inverted_t_indices
    
    
#     def check_stemi_nstmi(self, frequency):
      
#         temp_list = []

#         for lead in self.leads_pqrst_data.keys():
#             temp_list.append(self.leads_pqrst_data[lead]['ST_Label'])
#         abnormal_cou = temp_list.count('Abnormal')
#         st_ele_cou = temp_list.count('ST Segment Elevation')
#         st_dep_cou = temp_list.count('ST Segment Depression')
#         st_dep_up_cou = temp_list.count('ST Segment Depression upsloping')
#         st_dep_dn_cou = temp_list.count('ST Segment Depression downsloping')
#         st_dep_ho_cou = temp_list.count('ST Segment Depression horizontal')
#         #print("st_ele_cou: ",st_ele_cou, "abnormal_cou: ",abnormal_cou, "st_dep_cou: ", st_dep_cou, "st_dep_up_cou: ", st_dep_up_cou, "st_dep_dn_cou: ", st_dep_dn_cou, "st_dep_ho_cou: ", st_dep_ho_cou)
#         st_label = 'Abnormal'
        
#         if abnormal_cou >= 3:
#             st_label = 'Abnormal'
#         elif st_ele_cou >= 5 or st_dep_up_cou > 4:
#             st_label = 'STEMI'
#         elif st_dep_cou >= 5 or st_dep_dn_cou >= 2 or  st_dep_ho_cou >= 2 or (st_dep_cou == 2 and st_dep_dn_cou == 1) or (st_dep_cou == 2 and st_dep_ho_cou == 1):
#             st_label = 'NSTEMI'


#         lead_II = self.leads_pqrst_data['II']

#         Final_label = 'Abnormal'

#         lbbb_rbbb_label = self.check_LBBB_or_RBBB(lead_II['baseline_signal'], lead_II['r_index'])

#         if lbbb_rbbb_label != 'Abnormal':
#             if lbbb_rbbb_label == 'LBBB': 
#                 if st_label == 'STEMI':
#                     Final_label = f"Acute MI with LBBB"
#                 else:
#                     Final_label = lbbb_rbbb_label
#             elif lbbb_rbbb_label == 'RBBB':
#                 if st_label == 'STEMI':
#                     Final_label = f"Acute MI with RBBB"
#                 else: 
#                     Final_label = lbbb_rbbb_label
#         elif st_label == 'STEMI':
#             mi_label = self.anterior_posterior_detect(lead_II['baseline_signal'], lead_II['r_index'], lead_II['s_index'], lead_II['q_index'], lead_II['t_index'], frequency)
            
#             if mi_label != 'Abnormal':
#                 Final_label = f"{mi_label} {st_label}"
#             if st_dep_up_cou >= 3:
#                 Final_label = f"{st_label}"
#             elif mi_label != 'Abnormal' and lbbb_rbbb_label == "Abnormal":
#                 Final_label = f"{mi_label} {st_label}"
#             else:
#                 Final_label = st_label
#         elif st_label == 'NSTEMI':
#             Final_label = st_label
#             inv_t_label = self.detect_inv_t(lead_II['baseline_signal'], lead_II['r_index'], lead_II['s_index'], lead_II['q_index'], lead_II['t_index'])
#             if inv_t_label != 'Abnormal':
#                 Final_label = f"NSTEMI({inv_t_label})"
#         elif st_label == 'Abnormal':
#             mi_label = self.anterior_posterior_detect(lead_II['baseline_signal'], lead_II['r_index'], lead_II['s_index'], lead_II['q_index'], lead_II['t_index'], frequency)
#             if mi_label != 'Abnormal':
#                 Final_label = f'T wave Abnormality(Acute MI)'
#         return Final_label
    
#     def check_MI(self):
#         rest_all = self.check_stemi_nstmi(self.fs)
#         #print(rest_all)
#         la_teral = self.check_lateral_STEMI()
#         inferior = self.check_inferior_STEMI()
#         # Get `II` data
#         lead = 'II'
#         r_index = self.leads_pqrst_data[lead]['r_index']
#         baseline_signal = self.leads_pqrst_data[lead]['baseline_signal']
#         lowpass_signal = self.leads_pqrst_data[lead]['lowpass_signal']
#         final_label = '; '.join([i for i in [rest_all, la_teral, inferior] if i != 'Abnormal'])
#         return final_label if final_label else 'Normal'

# ----------------------------------- MI detection model base -----------------------------------------------

def count_st_seg(low_ecg_signal):
    fs = 200
    pqrst_data = pqrst_detection(low_ecg_signal, fs=fs)
    r_index = pqrst_data['R_index']
    s_index = pqrst_data['S_Index']
    pt_plot = pqrst_data['PT PLot']
    hr_ = pqrst_data['HR_Count']

    amplification_factor = 2
    scaler = MinMaxScaler(feature_range=(0, 4))
    normalized_ecg_data = scaler.fit_transform(low_ecg_signal.reshape(-1, 1)).flatten()

    amplified_ecg_data = normalized_ecg_data * amplification_factor
    duration_seconds = len(low_ecg_signal) / fs

    # Calculate the baseline value (isoelectric line)
    baseline_value = np.median(amplified_ecg_data)
    offset = 10.5 - baseline_value  # Adjust offset to align the baseline with the isoelectric line

    box_counts_ri = []
    box_counts_si = []

    for i in range(len(r_index)):
        r_val = amplified_ecg_data[r_index[i]] + offset
        s_val = amplified_ecg_data[s_index[i]] + offset

        vertical_distance_ri = abs(r_val - 10.5)
        box_count_ri = round(vertical_distance_ri, 2)
        box_counts_ri.append(box_count_ri)

        vertical_distance_si = abs(s_val - 10.5)
        box_count_si = round(vertical_distance_si, 2)
        box_counts_si.append(box_count_si)

    #print("======Box counts from isoelectric line to S points:", box_counts_si)
    avg_st = sum(box_counts_si)/ len(box_counts_si)
    return avg_st
...
def check_mi_model(all_leads_data, imageresource):
    ...
    for lead in all_leads_data.columns:
        ...
        if lead in ['II', 'III', 'aVF', 'I', 'aVL', 'v5']:
            ...
            st_seg = count_st_seg(lowpass_signal)
            if mi_result == 'STELE' or mi_result == 'STDEP':
                if mi_result == 'STELE':
                    mi_final_result = 'ST Segment Elevation'
                if mi_result == 'STDEP':
                    mi_final_result = 'ST Segment Depression'
                st_result.append({'Lead': lead, 'st_result': mi_final_result, 'MM': st_seg})
            st_mm_cou.append(st_seg)
    ...
    if st_mm_cou:
      avg_st_seg_cou = round(sum(st_mm_cou)/len(st_mm_cou))
    
    if counts['STELE'] >= 2:
      mi_result = 'STELE'
    if mi_result != 'STELE' and counts['STDEP'] >= 2:
      mi_result = 'STDEP'
      
    repeated_elements = [item for item, count in counts.items() if count >= 3]
    mi_t_lab = ' '.join(repeated_elements)
    if mi_t_lab == "TAB" and mi_result != "STELE" and mi_result != "STDEP":
        mi_result = "T_wave_Abnormality"
    print(avg_st_seg_cou,"===avg_st_seg_cou")
    return mi_result, avg_st_seg_cou, st_result
...
#                                    if rrints == "IRREGULAR" and rrint=="IRREGULAR" and int(timetaken)>6:
#                                        try:
...
#                                        if(SACompare(SAf, 4000)) and rrints == "IRREGULAR" or rrints == "REGULAR":
...
#                                            noofpause = 0
#
#                                        result_data.update({"Arrhythmia":'Long Pause',"noOfPause":noofpause,"noOfPauseList":[a/1000 for a in SAf if a>3000]})
#                                        d1 = result_data
#                                        finddata.append(d1)
#                                    if SACompareShort(SAf,2500,3500):
#                                        l=[]
#                                        for x in SAf:
...
##                                            plt.ioff()
                                        ...
                                        if actaulPVC.count(1)>0:
                                            result_data.update({"Arrhythmia":'PVC-Isolated',"Vbeats":actaulPVC.count(1),"HR":int(HR),"peakslocation":peaksdefined})
                                        if Quadgem_count>=1:
                                        ...
                                        if float(HR)<=60.0:
                                            ...
                                            if wideq["wideqrs_label"]=="Wide_QRS" and result_data["Arrhythmia"] not in ["PVC-Isolated", "PVC-Quadrigeminy", "PVC-Trigeminy","PVC-Bigeminy","PVC-Couplet","PVC-Triplet","VT","IVR","NSVT","AFIB","AFL"]:
                                                ...
                                                if widelens>5 and (1 in newpvcswide):
                                                    result_data.update({"Arrhythmia":'Normal',"Count":widelens,"peakslocation":peaksdefined})
                                                    d3 = result_data
                                                    finddata.append(d3)
...
##                                                    with tf.device('/GPU:0'):
                                                    predictions,ids = prediction_model_PAC(resized_image)
                                                    #print(predictions,ids)
...
##                                                updated_union = pac_new_model_check(naa, pac_load_model, fs=200)
##                                                
##                                                if updated_union==[]:
##                                                    [0,0,0,0,0,0,0,0]
##                                                else:
##                                                    pass
                                            ...
                                            if actaulPAC.count(1)>0 and forPAC>6:
                                                result_data.update({"Arrhythmia":'PAC-Isolated',"PACTOTALCOUNT":actaulPAC.count(1),"HR":int(HR)})
                                                d3 = result_data
                                                finddata.append(d3)
...
#                                            final_label,ei_ti_block = block_model_check(block_na, 200, labelss,newsublist)
#                                            print(final_label)
                                    ...
                                    try:
                                        ...
                                        if int(HR)<100 and int(timetaken)>6 and int(version) == 5 and rrint=="REGULAR":
                                            ...
                                            if len(all_lead_data) != 0 and len(all_lead_data['II'].values) > 500:
                                                ...
                                                label_mi, avg_st_cou, st_result = check_mi_model(all_lead_data, imageresource)
#                                                if label_rlbbb == "LBBB":
...
#                                                   finddata.append(d3)
                                                if label_mi == "STELE":
                                                   result_data.update({"Arrhythmia":'ABNORMAL',"TMT":{"MI":"ST Segment Elevation", 'MM': avg_st_cou, 'LeadResult':st_result}})
                                                   d3 = result_data
                                                   finddata.append(d3)
                                                elif label_mi == "STDEP":
                                                   result_data.update({"Arrhythmia":'ABNORMAL',"TMT":{"MI":"ST Segment Depression", 'MM': avg_st_cou, 'LeadResult':st_result}})
                                                   d3 = result_data
                                                   finddata.append(d3)
